...
namespace mjaang_bugtracker.Controllers
{
    ...
    public class AccountController : MyBaseController
    {
        ...
        {
            var result = await SignInManager.PasswordSignInAsync(uName, "Admin@123", model.RememberMe, shouldLockout: false);
            switch (result)
            {
                case SignInStatus.Success:
                    return RedirectToAction("Index", "Projects"); //User Dashboard
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.Failure:
                default:
